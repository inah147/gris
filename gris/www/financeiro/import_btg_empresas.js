  ...
  function renderResults(payload) {
    ...
    if (errors.length) {
      const items = errors
        ...
        .join('');
      const more = errors.length > 50
        ? `<p class="import-errors__more">+${escapeHtml(errors.length - 50)} erros adicionais. Consulte o Error Log para a lista completa.</p>`
        : '';
  ...
  window.sendBtgFile = function () {
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        showToast('success', 'Conciliação concluída', 'O extrato foi processado com sucesso.');
      },
      error: function (err) {
        if (loadingIndicator) loadingIndicator.classList.add('hidden');
        if (btnConciliar) btnConciliar.disabled = false;
        console.error('Erro ao processar BTG Empresas:', err);
        showToast('error', 'Erro na conciliação', 'Ocorreu um erro ao processar o arquivo.');
      },
    });
  };
