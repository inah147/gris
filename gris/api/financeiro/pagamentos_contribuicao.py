...
def apurar(meses=MESES_PADRAO, hoje: datetime.date | None = None, incluir_dados_cobranca: bool = False) -> dict:
    ...
    return {
        ...
        "totais": {
            ...
            "inadimplencia_associados": (
                round(len([a for a in associados if a["situacao"] == STATUS_ATRASADO]) / len(associados) * 100, 2)
                if associados
                else 0.0
            ),
