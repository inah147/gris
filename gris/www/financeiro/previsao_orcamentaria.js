...
(function () {
	...
	function setChartMessage(id, text) {
		...
		const target = qs(id);
		if (target)
			target.innerHTML = `<div class="text-muted-foreground text-sm px-2 pt-3">${text}</div>`;
	}
	...
	function renderPrevistoRealizado({
		...
		const temDado =
			(previsto || []).some((v) => parseNumber(v) !== 0) ||
			(realizado || []).some((v) => parseNumber(v) !== 0);
	...
	async function carregarComparativo() {
		...
		try {
			const r = await frappe.call({
				method: `${API}.obter_comparativo`,
				args: { previsao: previsaoAtual },
			});
			dados = r.message;
		} catch (e) {
			CHART_IDS.forEach((id) =>
	...
	async function salvarPrevisao(botao) {
		...
		if (!valores.titulo || !valores.exercicio || !dataInicio || !dataFim) {
			frappe.msgprint(__("Preencha título, exercício e o período da previsão."));
			return;
		}
		const args = {
	...
	async function salvarItem(botao) {
		...
		if (!valores.descricao || !valores.valor_previsto) {
			frappe.msgprint(__("Informe a descrição e o valor previsto do item."));
			return;
		}
		if (distribuicao === "Mês específico" && !mesReferencia) {
			frappe.msgprint(__("Informe o mês de referência do item."));
			return;
		}
		botao.disabled = true;
		try {
	...
	async function confirmarDuplicar(botao) {
		...
		if (!valores.titulo || !valores.exercicio || !dataInicio || !dataFim) {
			frappe.msgprint(__("Preencha título, exercício e o período da nova previsão."));
			return;
		}
		botao.disabled = true;
		try {
