...
function chamarAssociado() {
    ...
    frappe.confirm('Tem certeza que deseja chamar este associado? Ele sairá da fila de espera.', () => {
        frappe.call({
            method: "gris.www.recepcao.fila_espera.chamar_associado",
            args: { fila_id: currentFilaId },
            callback: function(r) {
                if (!r.exc) {
                    frappe.show_alert({message: 'Associado chamado com sucesso', indicator: 'green'});
                    closeModal();
...
function registrarDesistencia() {
    ...
    }, 'Registrar Desistência', 'Confirmar');
}
