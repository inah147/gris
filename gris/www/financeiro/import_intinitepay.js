(function () {
	...
	function checkShowConciliarBtn() {
		...
		const ok = !!(
			window._extratoFileUrl &&
			window._vendasFileUrl &&
			window._recebimentosFileUrl
		);
		if (btn) {
	...
	setupUploader(
		"ipRecebimentosUpload",
		"file-info-recebimentos",
	...
	window.enviarArquivosImportados = function () {
		...
		if (!window._extratoFileUrl || !window._vendasFileUrl || !window._recebimentosFileUrl) {
			showToast(
				"error",
				"Arquivos ausentes",
				"Faça o upload dos três arquivos antes de enviar."
			);
			return;
		}
		...
		frappe.call({
			...
			callback: function (r) {
				...
				if (!payload || !payload.stats) {
					const detalhe =
						(payload && payload.summary_text) ||
						"Verifique os arquivos enviados e tente novamente.";
				...
				showToast(
					...
					"Os arquivos Infinitepay foram processados com sucesso."
				);
			},
			error: function (err) {
				if (loadingIndicator) loadingIndicator.classList.add("hidden");
