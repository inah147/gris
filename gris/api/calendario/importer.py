...
def extract_events_from_text(content):
    ...
    for line in lines:
        ...
        if not ':' in line_stripped:
            if not re.match(r'\w+/\d{4}', line_stripped) and not re.match(r'^\d+(\s+e\s+\d+)?$', line_stripped):
...
def parse_calendario_report(path_pdf: str) -> dict:
    ...
    results = {"total": len(eventos_df), "created": 0, "updated": 0, "skipped": 0, "errors": 0, "error_details": []}
    ...
    for _, evento in eventos_df.iterrows():
        ...
        try:
            ...
            results["errors"] += 1
            results["error_details"].append(f"Erro ao importar evento '{evento.get('atividade')}': {str(e)}")
            frappe.log_error(f"Erro importação calendário: {str(e)}")
