...
(() => {
    ...
    function basecoatSelectHtml(idBase, niveis, valorAtual, dataAttrs) {
        const dataAttrsStr = Object.entries(dataAttrs || {})
            ...
            .join(" ");
        const opts = niveis.map((n, i) => {
            const selected = n === valorAtual ? ' aria-selected="true"' : "";
            return `<div id="${idBase}-items-${i + 1}" role="option" data-value="${htmlEscape(n)}"${selected}>${htmlEscape(n)}</div>`;
        }).join("");
        return `
    ...
    function renderMembros(membros, podeGerir, niveis) {
        ...
        lista.innerHTML = membros.map((m, idx) => {
            const isOwner = m.is_owner;
            const selectId = `select-nivel-membro-${idx}`;
            const nivelControl = podeGerir && !isOwner
                ? basecoatSelectHtml(selectId, niveis, m.nivel_acesso, {
                    "data-member-update": "",
                    "data-user": m.user,
                })
                : `<span class="badge-${badgeVariant(m.nivel_acesso)}">${htmlEscape(m.nivel_acesso)}</span>`;
            const removeBtn = podeGerir && !isOwner
                ? `<button type="button" class="btn-sm-outline" data-member-remove data-user="${htmlEscape(m.user)}" aria-label="Remover ${htmlEscape(m.full_name)}">Remover</button>`
                : "";
            return `
                <li class="membros-quadro__item" data-user="${htmlEscape(m.user)}">
                    <div class="membros-quadro__user">
                        ...
                        <small>${htmlEscape(m.user)}${isOwner ? ' &middot; <em>criador</em>' : ''}</small>
                    </div>
    ...
    function aplicarRotuloNome(comboEl) {
        ...
        if (!valor) return;
        const opt = Array.from(comboEl.querySelectorAll('[role="option"]'))
    ...
    function initMembros(boardName, podeGerir) {
        ...
        if (btnAdicionar && comboUsuario && podeGerir) {
            btnAdicionar.addEventListener("click", async () => {
                const user = comboUsuario.querySelector(':scope > input[type="hidden"]')?.value || "";
                const nivel = selectNivel?.querySelector(':scope > input[type="hidden"]')?.value || "Editar";
                if (!user) {
    ...
    function init() {
        ...
        const kanban = new window.GrisKanbanTarefas("#taskKanban", {
            ...
            onAddComment: async (tarefaName, texto) => {
                const data = await callApi(METHODS_COMUM.addComment, { tarefa_name: tarefaName, texto });
                return { comentarios: data.comentarios || [] };
            },
            onEditComment: async (commentName, texto) => {
                const data = await callApi(METHODS_COMUM.editComment, { comentario_name: commentName, texto });
                return { comentarios: data.comentarios || [] };
            },
            onDeleteComment: async (commentName) => {
                const data = await callApi(METHODS_COMUM.deleteComment, { comentario_name: commentName });
                return { comentarios: data.comentarios || [] };
            },
        });
        ...
        if (modo === "projeto") {
            kanban.setHint("Arraste cards entre colunas, clique para editar ou crie novas tarefas.");
        }

        kanban.refresh().then(() => {
            // Abre o dialog da tarefa quando a URL traz ?tarefa=<name> (ex.: vindo
            // do popover de tarefas da topbar).
            const tarefaParam = (new URLSearchParams(window.location.search).get("tarefa") || "").trim();
