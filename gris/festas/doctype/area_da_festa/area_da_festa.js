...
async function fill_area_coord_from_link(frm, doctype_name, docname) {
...
frappe.ui.form.on("Area da Festa", {
	tipo_coord(frm) {
	...
	associado_coord(frm) {
		if (frm.doc.tipo_coord === "Associado" && frm.doc.associado_coord) {
			fill_area_coord_from_link(frm, "Associado", frm.doc.associado_coord);
		}
	},
	responsavel_coord(frm) {
		if (frm.doc.tipo_coord === "Responsavel" && frm.doc.responsavel_coord) {
			fill_area_coord_from_link(frm, "Responsavel", frm.doc.responsavel_coord);
		}
	},
});

// Handlers compartilhados para Membro Equipe Festa (registrados aqui e em barraca_da_festa.js)
