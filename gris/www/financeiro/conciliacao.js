...
(function () {
	...
	function traco(v) {
		return v === null || v === undefined || v === ""
			? '<span class="conciliacao-comparacao__vazio">—</span>'
			: esc(v);
	}
	...
	function renderComparacao(sistema, planilha) {
		const linhas = [
			...
			[
				"Descrição",
				sistema.descricao || sistema.descricao_reduzida,
				planilha ? planilha.descricao || planilha.descricao_reduzida : null,
				false,
			],
			...
			[
				"Centro de custo",
				sistema.centro_de_custo,
				planilha ? planilha.centro_de_custo : null,
				false,
			],
			[
				"Tipo",
				sistema.ordinaria_extraordinaria,
				planilha ? planilha.ordinaria_extraordinaria : null,
				false,
			],
		];
		...
		corpo.innerHTML = linhas
			.map(([campo, vSis, vPla, numerico]) => {
				...
				const pla = planilha
					? traco(vPla)
					: '<span class="conciliacao-comparacao__vazio">Selecione um candidato</span>';
	...
	async function carregarPendentes() {
		...
		try {
			...
			if (!itens.length) {
				lista.innerHTML =
					'<p class="m-0 text-sm text-muted-foreground">Nenhuma transação de sistema pendente.</p>';
				return;
			}
			...
			lista.innerHTML = itens
				.map((t) => {
					const neg =
						parseNumber(t.valor) < 0 ? " conciliacao-item__valor--negativo" : "";
					const desc = t.descricao || t.descricao_reduzida || "";
					return `<button type="button" class="conciliacao-item" role="option" aria-selected="false" data-id="${esc(
						t.name
					)}">
			...
			lista.innerHTML =
				'<p class="m-0 text-sm text-destructive">Erro ao carregar pendentes.</p>';
			console.error(e);
		}
	}
	...
	async function selecionarSistema(sistemaId) {
		...
		candDiv.innerHTML =
			'<p class="m-0 text-sm text-muted-foreground">Buscando candidatos…</p>';

		try {
			const resp = await frappe.call({
				method: API + ".get_candidatos_planilha",
				args: { sistema_id: sistemaId },
			});
            ...
            <input type="radio" name="conc-candidato" value="${esc(
				t.name
			)}" class="conciliacao-candidato__radio" />
            <span class="conciliacao-candidato__corpo">
              <span class="conciliacao-candidato__topo">
                <span class="conciliacao-item__valor conciliacao-valor-num">${fmtMoney(
					t.valor
				)}</span>
                <span class="conciliacao-item__data">${fmtData(dataDe(t))}</span>
              </span>
              <p class="conciliacao-candidato__descricao" title="${esc(desc)}">${
						esc(desc) || "—"
					}</p>
			...
			candDiv.innerHTML =
				'<p class="m-0 text-sm text-destructive">Erro ao buscar candidatos.</p>';
			console.error(e);
		}
	}
	...
	window.conciliarPar = async function () {
		if (!sistemaAtual || !candidatoAtual) {
			frappe.show_alert({
				message: "Selecione um candidato da planilha.",
				indicator: "orange",
			});
			return;
		}
