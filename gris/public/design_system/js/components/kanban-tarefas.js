  ...
  <div id="${popoverId}" class="datepicker-popover" data-datepicker-popover role="dialog" aria-modal="false" aria-label="Selecionar data" hidden>
	...
	class GrisKanbanTarefas {
		...
		constructor(target, options) {
			...
			this.currentUserFullName = String(
				this.options.currentUserFullName || this.currentUser
			);
		...
		_renderKanban() {
			...
			this.tarefas.forEach((task) => {
				const status = TASK_STATUS_ORDER.includes(task.status)
					? task.status
					: "Nao iniciado";
                    ...
                    <section class="task-column" data-task-column="${escapeHtml(status)}">
                        <header class="task-column__header">
                            <div class="task-column__heading">
                                <h4 class="task-column__title">${escapeHtml(
		...
		_renderCardHtml(task) {
			...
			if (this.mode === "pessoal") {
				const badgeLabel =
					task.board_badge_label ||
					(task.board_referencia_doctype === "User" ? "Pessoal" : "");
				const badgeTipo = task.board_badge_tipo || "";
				if (badgeLabel) {
					extraTopHtml = `<span class="task-board-badge task-card__board-badge" data-tipo="${escapeHtml(
						badgeTipo
					)}" title="${escapeHtml(badgeLabel)}">${escapeHtml(badgeLabel)}</span>`;
				}
			} else {
				...
				footerExtraHtml = `<span class="task-card__responsavel${cls}" title="${escapeHtml(
					label
		...
		_renderResponsavelCombobox(selectedValue) {
			...
			const items = [{ value: "", label: "Selecione" }].concat(
				Array.from(dedup.entries()).map(([value, label]) => ({ value, label }))
			);
		...
		_setDialogEditable(editable) {
			this.dialog
				?.querySelectorAll("[data-kanban-field], [data-kanban-comment-input]")
				.forEach((el) => {
			...
			const responsavelTrigger = this._dlg(
				"[data-kanban-responsavel-wrapper] .select > button"
			);
		...
		_refreshComposerState() {
			...
			if (!hasTask) {
				...
				hint.textContent =
					"Voce possui acesso de leitura para os comentarios desta tarefa.";
		...
		_renderComments() {
			...
			if (this.taskCommentsLoading) {
				list.innerHTML =
					'<div class="task-comments-empty">Carregando comentarios...</div>';
				return;
			}
			if (!this.activeTask?.name) {
				list.innerHTML =
					'<div class="task-comments-empty">Salve a tarefa para liberar o historico de comentarios.</div>';
				return;
			}
			if (!this.taskComments.length) {
				list.innerHTML =
					'<div class="task-comments-empty">Nenhum comentario registrado para esta tarefa.</div>';
				return;
			}

			const currentUserLc = String(this.currentUser || "").toLowerCase();
			list.innerHTML = this.taskComments
				.map((comment) => {
					...
					const owner = String(comment.owner || comment.author_email || "")
						.trim()
						.toLowerCase();
					const isAuthor = Boolean(
						commentName && owner && currentUserLc && owner === currentUserLc
					);
					const isEditing = Boolean(
						commentName && this.editingCommentName === commentName
					);
					...
					if (isEditing) {
                        ...
                        <article class="task-comment-item task-comment-item--editing">
                            <div class="task-comment-item__row">
                                <span class="task-comment-item__avatar" aria-hidden="true">${escapeHtml(
									initials
								)}</span>
                                <div class="task-comment-item__main">
                                    <header class="task-comment-item__header">
                                        <strong class="task-comment-item__author">${escapeHtml(
											author
										)}</strong>
                                        <span class="task-comment-item__time">${escapeHtml(
											timestamp
										)}</span>
                                    </header>
                                    <div class="task-comment-item__bubble task-comment-item__bubble--edit">
                                        <textarea class="textarea task-comment-item__edit-input" rows="4"
                                            data-kanban-comment-edit-input="${escapeHtml(
												commentName
											)}"
                                            placeholder="Edite o comentario">${escapeHtml(
												this.editingCommentDraft || fallback
											)}</textarea>
                                    </div>
                                    <div class="task-comment-item__actions task-comment-item__actions--edit">
                                        <button type="button" class="task-comment-item__action-link task-comment-item__action-link--primary"
                                            data-kanban-comment-action="save-edit" data-comment-name="${escapeHtml(
												commentName
											)}">Salvar</button>
                    ...
                    <div class="task-comment-item__actions">
                        ...
                        <button type="button" class="task-comment-item__action-link"
                            data-kanban-comment-action="edit" data-comment-name="${escapeHtml(
								commentName
							)}"
                        ...
                        <span class="task-comment-item__action-sep" aria-hidden="true">•</span>
                        <button type="button" class="task-comment-item__action-link"
                            data-kanban-comment-action="delete" data-comment-name="${escapeHtml(
								commentName
							)}"
		...
		async _saveEditComment(commentName) {
			...
			const input = this.dialog?.querySelector(
				`[data-kanban-comment-edit-input="${CSS.escape(commentName)}"]`
			);
			const texto = (input?.value || "").trim();
			if (!texto) {
		...
		_bindEvents() {
			...
			const commentBtn =
				this.commentButtonId && document.getElementById(this.commentButtonId);
			...
			this.container.addEventListener("drop", (event) => {
				...
				column.classList.remove("is-drop-target");
				const taskName =
					this.dragTaskName || event.dataTransfer?.getData("text/plain") || "";
			...
			this.dialog.addEventListener("click", (event) => {
				...
				const prazoTrigger = event.target.closest(
					"[data-kanban-prazo-wrapper] .datepicker-trigger"
				);
				if (prazoTrigger) {
