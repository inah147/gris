...
def get_context(context):
	if frappe.session.user == "Guest":
		frappe.local.flags.redirect_location = "/login?redirect-to=/projetos/aprovacao_projeto"
		raise frappe.Redirect

	if not user_has_access("/projetos/aprovacao_projeto"):
	...
	if not projeto_name:
		frappe.throw("Projeto não informado para aprovação.")
	...
	enrich_context(context, "/projetos/aprovacao_projeto")
	return context
