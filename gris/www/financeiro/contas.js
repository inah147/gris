...
(function () {
	...
	function rebuildSelect(selectId, items, currentValue) {
		...
		listbox.innerHTML = items.map((it, i) => {
			...
			return `<div id="${selectId}-items-${i + 1}" role="option" data-value="${it.value || ''}"${sel}>${it.label || ''}</div>`;
		}).join('');
		const selected = items.find(it => it.value === currentValue) || items[0] || { label: '', value: '' };
		...
		document.addEventListener('click', function (e) {
			...
			new frappe.ui.FileUploader({
				...
				on_success(file) {
					...
					if (nomeSpan) { nomeSpan.textContent = file.file_name || file.name; nomeSpan.classList.remove('hidden'); }
	...
	window.fecharNovaInstituicaoModal = function () { closeDialog('novaInstituicaoModal'); };
	window.fecharNovaCarteiraModal = function () { closeDialog('novaCarteiraModal'); };
	window.fecharCarteiraModal = function () { closeDialog('carteiraDetalheModal'); };
	window.fecharImportarDadosModal = function () { closeDialog('importarDadosModal'); };
	...
	async function salvarCampos() {
		...
		for (const [field, value] of Object.entries(updates)) {
			try {
				await frappe.call({ method: 'frappe.client.set_value', args: { doctype: 'Carteira', name, fieldname: field, value } });
	...
	document.addEventListener('click', function (e) {
		...
		if (importarBtn) {
			...
			if (nome === 'infinitepay') { window.location.href = '/financeiro/import_intinitepay'; return; }
			if (nome === 'portão 3' || nome === 'portao 3') { window.location.href = '/financeiro/import_portao3'; return; }
			if (nome === 'btg empresas' || nome === 'btgempresas' || nome === 'btg') { window.location.href = '/financeiro/import_btg_empresas'; return; }
		...
		if (detalhesBtn) {
			...
			qs('carteiraInputPix').value = (detalhesBtn.dataset.chavePix && detalhesBtn.dataset.chavePix !== '—') ? detalhesBtn.dataset.chavePix : '';
			qs('carteiraInputDescricao').value = (detalhesBtn.dataset.descricao && detalhesBtn.dataset.descricao !== 'Sem descrição.') ? detalhesBtn.dataset.descricao : '';
			setEditMode(false);
			return;
		}
		...
		if (e.target.closest('#btnSalvarCarteiraNova')) {
			(async function () {
				...
				try {
					const r = await frappe.call({ method: 'frappe.client.insert', args: { doc: { doctype: 'Carteira', nome, instituicao_financeira: instituicao, descricao, responsavel, centro_de_custo: centro, chave_pix: pix } } });
					const doc = (r && r.message) || r;
					if (doc && doc.name) {
		...
		if (e.target.closest('#btnEditarCarteira')) {
			...
			Promise.all([fetchCentroCustoOptions(), fetchUsers()]).then(([centros, users]) => {
				...
				rebuildSelect('carteiraInputCentro', buildItems(centros, '—'), curCentro === '—' ? '' : curCentro);
				rebuildSelect('carteiraInputResponsavel', buildItems(users, '—'), curResp === '—' ? '' : curResp);
			});
			return;
		}
