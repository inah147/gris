...
def aggregate_results(results: dict) -> dict:
	...
	delta_pass_rate = primary.get("pass_rate", {}).get("mean", 0) - baseline.get("pass_rate", {}).get(
		"mean", 0
	)
	delta_time = primary.get("time_seconds", {}).get("mean", 0) - baseline.get("time_seconds", {}).get(
		"mean", 0
	)
...
def generate_markdown(benchmark: dict) -> str:
	...
	lines = [
		...
		f"**Evals**: {', '.join(str(e) for e in metadata['evals_run'])} "
