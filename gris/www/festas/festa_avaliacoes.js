(function () {
    ...
    function callApi(method, args) {
        return new Promise(function (resolve, reject) {
            frappe.call({
                ...
                error: function (err) {
                    reject(new Error((err && err.message) || "Erro de comunicação com o servidor."));
                },
            });
        });
    }
    ...
    function renderIndividuais(avaliacao, data) {
        ...
        if ($("avaliacaoProgressText")) {
            $("avaliacaoProgressText").textContent = concluidas + " de " + total + " avaliações concluídas";
        }
        ...
        if ($("avaliacaoMetricSatisfacao")) {
            $("avaliacaoMetricSatisfacao").textContent =
                concluidas > 0 ? Number(avaliacao.satisfacao_dos_participantes || 0).toFixed(2) : "-";
        }

        renderResumo("avaliacaoResumoIndividual", "avaliacaoResumoIndividualContent", "btnGerarResumoIndividual",
            avaliacao.resumo_avaliacoes_individuais, concluidas > 0, data.can_edit_general);

        var listEl = $("avaliacaoAvaliadoresList");
        if (listEl) {
            listEl.innerHTML = individuais.map(function (a) {
                var badge = a.avaliacao_concluida ? "badge badge-success" : "badge badge-secondary";
                var label = a.avaliacao_concluida ? "Concluída" : "Pendente";
                var actions = "";
                if (a.avaliacao_concluida) {
                    actions = '<button type="button" class="btn-sm-outline" data-ver-avaliacao-idx="' + a.idx + '">Ver detalhes</button>';
                } else if (data.can_edit_general) {
                    actions = '<button type="button" class="btn-sm-outline" data-reenviar-idx="' + a.idx + '">Reenviar e-mail e WhatsApp</button>';
                }
                return (
                    ...
                    '<div class="aval-avaliador__actions">' + actions + "</div></div>"
                );
            }).join("");
        }
    }
    ...
    function renderGeral(avaliacao, data) {
        ...
        renderResumo("avaliacaoResumoCompleto", "avaliacaoResumoCompletoContent", "btnGerarResumoCompleto",
            avaliacao.resumo_avaliacao_completa, true, data.can_edit_general);
    }
    ...
    function renderConvidados(avaliacao, data) {
        ...
        if ($("avaliacaoMetricRecomendacao")) {
            $("avaliacaoMetricRecomendacao").textContent =
                convidados.length > 0 ? Number(avaliacao.recomendacao_media_convidados || 0).toFixed(1) : "-";
        }
        ...
        if (listEl) {
            if (convidados.length === 0) {
                listEl.innerHTML = '<p class="text-sm text-muted-foreground">Nenhuma avaliação de convidado recebida ainda.</p>';
            } else {
                listEl.innerHTML = convidados.map(function (c) {
                    return (
                        '<div class="aval-avaliador">' +
                        '<div class="aval-avaliador__info">' +
                        '<span class="aval-avaliador__nome">' + escapeHtml(c.email || "—") + "</span>" +
                        '<span class="badge badge-secondary">Nota ' + escapeHtml(c.recomendacao) + "</span></div>" +
                        '<div class="aval-avaliador__actions">' +
                        '<button type="button" class="btn-sm-outline" data-ver-convidado-idx="' + c.idx + '">Ver detalhes</button>' +
                        "</div></div>"
                    );
                }).join("");
            }
        ...
        renderResumo("avaliacaoResumoConvidados", "avaliacaoResumoConvidadosContent", "btnGerarResumoConvidados",
            avaliacao.resumo_avaliacoes_convidados, convidados.length > 0, data.can_edit);
    }
    ...
    function iniciarAvaliacao() {
        ...
        callApi(METHODS.iniciar, { festa_name: FESTA_NAME })
            ...
            .catch(function (err) {
                ...
                if (btn) { btn.disabled = false; btn.textContent = "Iniciar avaliação da equipe"; }
            });
    }
    ...
    function salvarAvaliacaoGeral() {
        ...
        callApi(METHODS.salvarGeral, { festa_name: FESTA_NAME, data: JSON.stringify(data) })
            ...
            .finally(function () {
                state.saving = false;
                if (btn) { btn.disabled = false; btn.textContent = "Salvar avaliação geral"; }
            });
    }
    ...
    function reenviarConvite(idx) {
        ...
        callApi(METHODS.reenviar, { festa_name: FESTA_NAME, avaliador_idx: idx })
            ...
            .catch(function (err) {
                ...
                if (btn) { btn.disabled = false; btn.textContent = "Reenviar e-mail e WhatsApp"; }
            });
    }
    ...
    function gerarResumo(tipo) {
        ...
        var btn = $(cfg.btn);
        if (btn) { btn.disabled = true; btn.textContent = "Gerando resumo..."; }
    ...
    function abrirDetalheAvaliador(idx) {
        var item = ((state.data && state.data.avaliacao && state.data.avaliacao.individuais) || []).find(
            function (a) { return String(a.idx) === String(idx); }
        );
        if (!item) return;
    ...
    function abrirDetalheConvidado(idx) {
        var item = ((state.data && state.data.avaliacao && state.data.avaliacao.convidados) || []).find(
            function (c) { return String(c.idx) === String(idx); }
        );
        if (!item) return;
        ...
        if (contentEl) {
            contentEl.innerHTML =
                ...
                campos.map(function (c) {
                    return '<div class="aval-detalhe-item"><span class="aval-detalhe-item__label">' +
                        c[0] + '</span><p class="aval-detalhe-item__value">' + c[1] + "</p></div>";
                }).join("") +
                "</div>";
        }
    ...
    function copiarLink() {
        ...
        navigator.clipboard.writeText(input.value).then(
            function () { toast("Link copiado!", "success"); },
    ...
    function bind() {
        ...
        document.addEventListener("click", function (event) {
            ...
            if (verAval) return abrirDetalheAvaliador(verAval.getAttribute("data-ver-avaliacao-idx"));

            var verConv = target.closest("[data-ver-convidado-idx]");
            if (verConv) return abrirDetalheConvidado(verConv.getAttribute("data-ver-convidado-idx"));
        });
    }
