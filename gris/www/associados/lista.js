...
frappe.ready(() => {
  ...
  function loadColVisibility() {
    ...
    try {
      ...
      if (saved && typeof saved === 'object') {
        ...
        COLUMNS.filter((c) => c.always).forEach((c) => { saved[c.key] = true; });
  ...
  function saveColVisibility(vis) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(vis)); } catch (_) { /* ignore */ }
  }
    ...
    listEl.innerHTML = rows.map((row) => {
      ...
      return `
        <tr class="assoc-table-row" data-href="${link}">
          ...
          ${v.validade_registro ? `<td><span class="text-muted-foreground">${formatDate(row.validade_registro) || '—'}</span></td>` : ''}
  ...
  async function fetchList() {
    ...
    try {
      const r = await frappe.call({
        ...
        args: {
          doctype: 'Associado',
          fields: ['name', 'nome_completo', 'registro', 'status', 'ramo', 'categoria', 'funcao', 'area', 'secao', 'validade_registro'],
          filters,
