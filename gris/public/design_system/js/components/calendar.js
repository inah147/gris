...
(() => {
		...
		const card = el("button", {
			...
			attrs: {
				...
				"aria-label": `${event.title}${
					formatTimeRange(event, options.locale)
						? ", " + formatTimeRange(event, options.locale)
						: ""
				}`,
			},
			style: {
				"--cal-cat-color":
					event.color || options.colorOf?.(event.category) || "var(--primary)",
			},
		});
		...
		if (showTime) {
			card.appendChild(
				el("span", {
					...
					text: formatTimeRange(event, options.locale),
				})
			);
		}
	...
	const createWeekAllDayBand = ({
		...
		const band = el("div", {
			class: showAxisLabel
				? "calendar__week-allday"
				: "calendar__week-allday calendar__week-allday--no-axis",
		});

		if (showAxisLabel) {
			band.appendChild(
				el("div", { class: "calendar__week-allday-label", text: "Dia inteiro" })
			);
		}
				...
				el("div", {
					class: [
						...
						isWeekendDate(dayDate)
							? "calendar__week-allday-cell--weekend"
							: "calendar__week-allday-cell--weekday",
					].join(" "),
				})
			);
		}
		...
		const bandHeight = Math.max(
			cardHeight + 8,
			8 + laneCount * cardHeight + Math.max(0, laneCount - 1) * gap
		);
	...
	const initCalendar = (root) => {
		...
		const listShowAllDaysCheckbox = root.querySelector(
			"[data-calendar-list-show-all-days-control] input[type='checkbox']"
		);

		// -------- State -----------------------------------------------------

		const allowedModes = config.allowedModes || ["month", "week", "list"];
		const initialMode = allowedModes.includes(config.initialMode)
			? config.initialMode
			: allowedModes[0];
		...
		let mode =
			!config.initialMode && isMobile() && allowedModes.includes("list")
				? "list"
				: initialMode;
		let anchorDate = config.initialDate
			? parseISO(config.initialDate)
			: startOfDay(new Date());
		...
		const hourRange =
			Array.isArray(config.hourRange) && config.hourRange.length === 2
				? config.hourRange
				: [0, 24];
		...
		const getListPeriodLabel = () => {
			...
			if (sameMonth) {
				return capitalize(
					getFormatter(locale, { month: "long", year: "numeric" }).format(start)
				);
			}
			if (sameYear) {
				const startLabel = capitalize(
					getFormatter(locale, { month: "short" }).format(start).replace(".", "")
				);
				const endLabel = capitalize(
					getFormatter(locale, { month: "short" }).format(end).replace(".", "")
				);
				return `${startLabel} – ${endLabel} ${start.getFullYear()}`;
			}

			const rangeFormatter = getFormatter(locale, {
				day: "numeric",
				month: "short",
				year: "numeric",
			});
		...
		const buildListDays = (visibleEvents) => {
			...
			for (const day of ordered) {
				day.items.sort((a, b) => {
					...
					return (
						a.start - b.start || String(a.title).localeCompare(String(b.title), locale)
					);
				});
			}
		...
		const createListDateColumn = (date) => {
			...
			dateCol.appendChild(
				el("span", {
					...
					text: capitalize(
						getFormatter(locale, { weekday: "short" }).format(date).replace(".", "")
					),
				})
			);
			dateCol.appendChild(
				el("span", { class: "calendar__list-date-number", text: String(date.getDate()) })
			);
			return dateCol;
		};
		...
		const createListEventItem = (event) => {
			...
			item.appendChild(
				el("div", { class: "calendar__list-event-title", text: event.title })
			);
			const time = formatTimeRange(event, locale);
			if (time)
				item.appendChild(el("div", { class: "calendar__list-event-time", text: time }));
			return item;
		};

		const createListEmptyState = (text = "Sem eventos") =>
			el("div", {
		...
		const renderMonth = () => {
			...
			setPeriodLabel(
				capitalize(getFormatter(locale, { month: "long", year: "numeric" }).format(first))
			);
			...
			for (let i = 0; i < 7; i += 1) {
				...
				const label = capitalize(
					getFormatter(locale, { weekday: "short" }).format(sample).replace(".", "")
				);
				weekdayHeader.appendChild(
					el("div", { class: "calendar__month-weekday", text: label })
				);
			}
			...
			for (let w = 0; w < 6; w += 1) {
				...
				for (let d = 0; d < 7; d += 1) {
					...
					const cell = el("div", {
						...
						attrs: {
							role: "gridcell",
							"aria-label": getFormatter(locale, {
								day: "numeric",
								month: "long",
								year: "numeric",
							}).format(dayDate),
				...
				for (const { cell, dayDate, hiddenEvents } of dayCells) {
					...
					const popover = el("div", {
						class: "calendar__more-popover",
						attrs: { role: "dialog" },
					});
					popover.appendChild(
						el("div", {
							class: "calendar__more-popover-day",
							text: getFormatter(locale, { day: "numeric", month: "long" }).format(
								dayDate
							),
						})
					);
					...
					moreBtn.addEventListener("click", (ev) => {
						...
						body.querySelectorAll('[data-more-popover-open="true"]').forEach((c) => {
							...
							c.querySelector("[data-calendar-more-toggle]")?.setAttribute(
								"aria-expanded",
								"false"
							);
						});
		...
		const renderWeek = () => {
			...
			setPeriodLabel(
				`${getFormatter(locale, { day: "numeric", month: "short" }).format(
					weekStart
				)} – ${getFormatter(locale, {
					day: "numeric",
					month: "short",
			...
			const wrap = el("div", {
				class: "calendar__week" + (useHours ? "" : " calendar__week--no-hours"),
			});
			...
			const header = el("div", {
				class:
					"calendar__week-header" + (useHours ? "" : " calendar__week-header--no-axis"),
			});
			...
			for (let d = 0; d < 7; d += 1) {
				...
				const cell = el("div", {
					class: [
						...
						isWeekend
							? "calendar__week-day-header--weekend"
							: "calendar__week-day-header--weekday",
				...
				cell.appendChild(
					el("span", {
						class: "calendar__week-day-name",
						text: capitalize(
							getFormatter(locale, { weekday: "short" })
								.format(dayDate)
								.replace(".", "")
						),
					})
			...
			const allDayEvents = inWeek.filter(
				(e) => e.allDay || !e.startHasTime || isMultiDayEvent(e)
			);
			const timedEvents = inWeek.filter(
				(e) => !e.allDay && e.startHasTime && !isMultiDayEvent(e)
			);
			...
			if (useHours) {
				...
				const pxPerMin =
					parseFloat(getComputedStyle(root).getPropertyValue("--cal-px-per-min")) || 0.8;
				...
				for (let d = 0; d < 7; d += 1) {
					...
					const dayCol = el("div", {
						class: [
							...
							isWeekend
								? "calendar__week-day-column--weekend"
								: "calendar__week-day-column--weekday",
					...
					for (const event of timedEvents) {
						const evtStart = event.start;
						const evtEnd =
							event.end && event.endHasTime
								? event.end
								: new Date(
										evtStart.getTime() + DEFAULT_EVENT_DURATION_MIN * 60000
								  );
				...
				for (let d = 0; d < 7; d += 1) {
					...
					const cell = el("div", {
						class: [
							...
							isWeekend
								? "calendar__week-stacked-cell--weekend"
								: "calendar__week-stacked-cell--weekday",
			...
			if (days.length === 0) {
				body.replaceChildren(
					el("div", {
						...
						text: config.emptyMessage || "Nenhum evento.",
					})
				);
				return;
			}
				...
				if (monthKey !== lastMonthKey) {
					...
					monthBlock.appendChild(
						el("div", {
							...
							text: capitalize(
								getFormatter(locale, { month: "long", year: "numeric" }).format(
									date
								)
							),
						})
		...
		const renderListByCategory = (days, today, visibleEvents) => {
			...
			for (const day of days) {
				...
				if (monthKey !== lastMonthKey) {
					monthBlock = el("div", {
						class: "calendar__list-month calendar__list-month--by-category",
					});
					monthBlock.appendChild(
						el("div", {
							class: "calendar__list-month-header",
				...
				const row = el("div", {
					class: [
						...
						isWeekend
							? "calendar__list-category-row--weekend"
							: "calendar__list-category-row--weekday",
				...
				for (const descriptor of descriptors) {
					...
					if (categoryItems.length === 0) {
						...
						for (const event of categoryItems)
							cell.appendChild(createListEventItem(event));
					}
		...
		const render = () => {
			...
			if (listVariantToggle) {
				listVariantToggle
					.querySelectorAll("[data-calendar-list-variant]")
					.forEach((btn) => {
		...
		const closeAllMorePopovers = () => {
			body.querySelectorAll('[data-more-popover-open="true"]').forEach((cell) => {
				...
				cell.querySelector("[data-calendar-more-toggle]")?.setAttribute(
					"aria-expanded",
					"false"
				);
			});
		...
		body.addEventListener("click", (event) => {
			...
			if (card) {
				...
				if (!found) return;
				root.dispatchEvent(
					new CustomEvent("gris:calendar:event-click", {
						bubbles: true,
						detail: {
							id: found.id,
							title: found.title,
			...
			if (!date) return;
			root.dispatchEvent(
				new CustomEvent("gris:calendar:day-click", {
					bubbles: true,
					detail: { date },
				})
			);
		...
		const rebuildFilterOptions = () => {
			...
			for (const cat of categories) {
				...
				const label = el("span", {
					class: "calendar__filter-label",
					text: cat.label || cat.name,
				});
				popover.appendChild(
					el("label", { class: "calendar__filter" }, [input, swatch, label])
				);
			}
		};
