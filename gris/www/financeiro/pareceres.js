...
(function () {
	...
	window.closeParecerModal = function () {
		...
		if (publishBtn || unpublishBtn) {
			...
			frappe.call({
				...
				args: {
					...
					name: docname,
					fieldname: { publicado: isPublish ? 1 : 0 },
				},
				callback: function (r) {
					target.disabled = false;
					if (!r.exc) {
						frappe.show_alert({
							message: isPublish ? "Publicado" : "Despublicado",
							indicator: isPublish ? "green" : "orange",
						});
						const card = document.querySelector(
							'.parecer-card[data-name="' + docname + '"]'
						);
						if (card) {
	...
	async function saveNewParecer(btn) {
		...
		const trimestre = !trimestreWrapper.classList.contains("hidden")
			? selectValue("addParecerTrimestre")
			: "";
