...
(function () {
  ...
  frappe.ready(() => {
    ...
    function renderRows(rows) {
      if (!rows.length) {
        renderEmpty('Nenhuma resposta encontrada', 'Tente ajustar a busca ou aguarde novas respostas.');
        return;
      }
      tbody.innerHTML = rows
        .map((row) => {
          const responsavel = escapeHtml(row.responsavel_nome || row.responsavel || 'Desistência');
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (params) => {
            ...
            return `<strong>${escapeHtml(item.name)}</strong><br/>NPS: ${v.toFixed(1).replace('.', ',')}`;
          },
        },
        legend: { show: false },
        xAxis: {
