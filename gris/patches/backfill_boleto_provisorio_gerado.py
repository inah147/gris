...
def execute():
	"""Marca a etapa nova "boleto provisório gerado" em quem já efetivou o registro provisório.

	A etapa nasce desmarcada em todos os registros, e quem já está com o registro provisório
	efetivado apareceria no funil como boleto pendente — exatamente o contrário do que a etapa
	existe para mostrar. Um registro provisório efetivado só existe porque o boleto foi emitido
	...
	if not frappe.db.has_column("Novo Associado", "boleto_provisorio_gerado"):
		return

	novo_associado = frappe.qb.DocType("Novo Associado")
	(
		frappe.qb.update(novo_associado)
		.set(novo_associado.boleto_provisorio_gerado, 1)
		.where(novo_associado.registro_provisorio_efetivado == 1)
	).run()
