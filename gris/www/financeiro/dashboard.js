...
(async function () {
	...
	async function atualizarCardInadimplencia() {
		...
		try {
			const resp = await frappe.call({
				method: "gris.api.financeiro.dashboard.get_inadimplencia_historica_12m",
			});
	...
	async function fetchData(extraArgs) {
		...
		const r = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_entradas_saidas_mensal",
			args,
		});
		return r.message || {};
	}

	function renderMoneyStackedBar({
		id,
		labels,
		datasets,
		yAxisName,
	...
	function renderMoneyLineWithBand({
		id,
		labels,
		seriesName,
		values,
		...
		chart.setOption(
			{
				...
				tooltip: {
					...
					formatter: (params) => {
						const item = params[0];
						if (!item) return "";
						return `<strong>${item.axisValue}</strong><br/>${
							item.marker
						}${seriesName}: <strong>${formatCurrency(item.value)}</strong>`;
					},
				},
				series: [
					{
		...
		chart.setOption(
			{
				...
				tooltip: {
					...
					formatter: (params) => {
						const item = params[0];
						if (!item) return "";
						return `<strong>${item.axisValue}</strong><br/>${item.marker}${
							dsResultado.name || "Resultado"
	...
	async function renderEntradasCredito() {
		const data = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_entradas_credito_mensal",
			args: currentFilters,
		});
	...
	async function renderEntradasCreditoCategoria() {
		...
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_entradas_credito_mensal_por_categoria",
			args: filtersSemCategoria,
		});
	...
	async function renderEntradasCreditoCentro() {
		...
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_entradas_credito_mensal_por_centro_custo",
			args: filtersSemCentro,
		});
		const payload = resp.message || resp || {};
	...
	async function renderEntradasCreditoTipo() {
		...
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_entradas_credito_mensal_por_tipo",
			args: filtersSemTipo,
		});
		const payload = resp.message || resp || {};
	...
	async function renderSaidasDebito() {
		const data = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_saidas_debito_mensal",
			args: currentFilters,
		});
		const payload = data.message || data || {};
		const ds = (payload.datasets || [])[0];
	...
	async function renderSaidasDebitoCategoria() {
		const { categoria, ...filtersSemCategoria } = currentFilters;
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_saidas_debito_mensal_por_categoria",
			args: filtersSemCategoria,
		});
		const payload = resp.message || resp || {};
	...
	async function renderSaidasDebitoCentro() {
		const { centro_de_custo, ...filtersSemCentro } = currentFilters;
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_saidas_debito_mensal_por_centro_custo",
			args: filtersSemCentro,
		});
		const payload = resp.message || resp || {};
	...
	async function renderSaidasDebitoTipo() {
		const { ordinaria_extraordinaria, ...filtersSemTipo } = currentFilters;
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_saidas_debito_mensal_por_tipo",
			args: filtersSemTipo,
		});
		const payload = resp.message || resp || {};
	...
	async function renderContribuicoesMensaisInadimplencia() {
		const resp = await frappe.call({
			method: "gris.api.financeiro.dashboard.get_contribuicoes_mensais_inadimplencia",
		});
		const payload = resp.message || resp || {};
		const ds = (payload.datasets || [])[0];
