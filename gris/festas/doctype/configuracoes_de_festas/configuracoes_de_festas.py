...
class ConfiguracoesdeFestas(Document):
	def validate(self):
	...
	def _normalize_fields(self):
		self.drive_compartilhado_festas = (self.drive_compartilhado_festas or "").strip()
		self.pasta_festas_id = (self.pasta_festas_id or "").strip()

	def _validate_drive_selection(self):
		drive_map = _get_active_party_drive_map()

		if self.drive_compartilhado_festas and self.drive_compartilhado_festas not in drive_map:
			frappe.throw(
				_(
					"Drive compartilhado das festas invalido. Selecione um drive ativo em Configuracoes Google Workspace."
				)
			)

		if cint(self.habilitar_pastas_festas_drive) and not drive_map:
			frappe.throw(
				_("Nao ha drives ativos em Configuracoes Google Workspace para habilitar pastas de festas.")
			)

	def _validate_required_fields_when_enabled(self):
		if not cint(self.habilitar_pastas_festas_drive):
			return

		if not self.drive_compartilhado_festas:
			frappe.throw(_("Selecione o drive compartilhado das festas."))

		if not self.pasta_festas_id:
			frappe.throw(_("Informe o ID da pasta de festas."))


@frappe.whitelist()
def get_opcoes_drives_compartilhados_festas() -> list[dict[str, str]]:
