...
(() => {
	...
	function buildBadges(data) {
		...
		if (data.status) {
			...
			out.push(
				'<span class="badge ' + statusVariantClass(slug) + '">' + data.status + "</span>"
			);
		}
		out.push(
			'<span class="badge ' +
				(data.ativa ? "" : "badge-secondary") +
				'">' +
		...
		out.push(
			'<span class="badge ' +
				(data.temporaria ? "badge-info" : "") +
				'">' +
				(data.temporaria ? "Temporária" : "Contínua") +
				"</span>"
		);
		return out.join("");
	}
		...
		frappe.call({
			...
			callback: (r) => {
				...
				dados.forEach((p) => {
					...
					const badgeHtml =
						'<span class="badge ' +
						statusVariantClass(slug) +
						'">' +
						p.status +
						"</span>";
					tr.innerHTML =
						'<td class="whitespace-nowrap">' +
						(p.mes_format || p.mes_referencia || "") +
						"</td>" +
						"<td>" +
						badgeHtml +
						"</td>" +
	...
	function showCreateModal() {
		...
		if (form) form.reset();
		const ativa = qs("novaAtiva");
		if (ativa) ativa.checked = true;
		const temp = qs("novaTemporaria");
		if (temp) temp.checked = false;
		const cobrar = qs("novaCobrarMesAtual");
		if (cobrar) cobrar.checked = false;
	...
	function init() {
		...
		if (modalDespesa) {
			...
			if (editButtons) {
				...
				editButtons
					...
					.addEventListener("click", (e) => saveEdits(e.currentTarget));
			}
		}
		...
		if (modalNova) {
			...
			if (saveNovaBtn)
				saveNovaBtn.addEventListener("click", (e) => saveNovaDespesa(e.currentTarget));
		}
	}
