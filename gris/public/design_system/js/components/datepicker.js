(() => {
  ...
  const initDatepicker = (root) => {
    ...
    const updateLabel = () => {
      if (mode === "single") {
        ...
        if (state.start && state.end) {
          labelEl.textContent = `${formatDisplay(state.start, locale)} → ${formatDisplay(state.end, locale)}`;
          labelEl.classList.remove("datepicker-trigger__label--placeholder");
        } else if (state.start) {
    ...
    popover.addEventListener("keydown", (e) => {
      ...
      if (!current) return;
      let target = null;
      if (e.key === "ArrowLeft") target = new Date(current.getFullYear(), current.getMonth(), current.getDate() - 1);
      else if (e.key === "ArrowRight") target = new Date(current.getFullYear(), current.getMonth(), current.getDate() + 1);
      ...
      else if (e.key === "PageUp") target = new Date(current.getFullYear(), current.getMonth() - 1, current.getDate());
      else if (e.key === "PageDown") target = new Date(current.getFullYear(), current.getMonth() + 1, current.getDate());
      else if (e.key === "Home") target = new Date(current.getFullYear(), current.getMonth(), current.getDate() - current.getDay());
      else if (e.key === "End") target = new Date(current.getFullYear(), current.getMonth(), current.getDate() + (6 - current.getDay()));
      ...
      e.preventDefault();
      if (target.getMonth() !== state.cursor.getMonth() || target.getFullYear() !== state.cursor.getFullYear()) {
