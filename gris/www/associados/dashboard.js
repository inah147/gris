...
(async function () {
	...
	function setChartMessage(id, text) {
		...
		const target = document.getElementById(id);
		if (target)
			target.innerHTML = `<div class="text-sm text-muted-foreground chart-message">${text}</div>`;
	}
	...
	function renderEvasaoMensalChart(chartData) {
		...
		const dsTaxa =
			datasets.find((d) => /Taxa Evasão/i.test(d.name || "")) || datasets[1] || {};
				...
				tooltip: {
					...
					formatter: (params) => {
						const rows = params
							.map((item) => {
								const suffix =
									item.seriesName === (dsTaxa.name || "Taxa Evasão (%)")
										? "%"
										: "";
								return `${item.marker}${item.seriesName}: <strong>${formatNumber(
									item.value
								)}${suffix}</strong>`;
							})
							.join("<br/>");
	...
	async function refreshDashboard(filters) {
		try {
			...
			const r = await frappe.call({
				method: "gris.www.associados.dashboard.get_associados_dashboard",
				args: filters,
			});
