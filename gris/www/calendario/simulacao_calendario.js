...
function initNewEventDialog() {
    ...
    saveButton?.addEventListener("click", () => {
        ...
        const secoes = Array.from(document.querySelectorAll('input[name="modal-secao"]:checked')).map((item) => item.value);
...
function openEditDialog(data) {
    ...
    setFieldValueWhenReady(document.getElementById("edit-inicio"), normalizeDateValue(data.inicio));
    setFieldValueWhenReady(document.getElementById("edit-termino"), normalizeDateValue(data.termino));
...
function initReconciliation() {
    ...
    selectAllButton?.addEventListener("click", () => {
        tbody.querySelectorAll('[data-reconcile-switch-index] input[type="checkbox"]').forEach((input) => {
            input.checked = true;
        });
    });
    ...
    function renderDifferences(differences) {
        ...
        confirmButton.onclick = () => {
            ...
            differences.forEach((diff, index) => {
                ...
                if (diff.type === "modified") {
                    actions.push({ action: "update", name: diff.key, doc: diff.simulated, sim_name: diff.key });
                }
            });
    ...
    frappe.call({
        ...
        callback(result) {
            if (result.message?.count !== undefined) {
                ...
                document.getElementById("success-message").textContent = `${result.message.count} alterações aplicadas com sucesso.`;
...
function openHolidayDialog(data) {
    ...
    document.getElementById("holiday-modal-type").dataset.type = (data.holiday_type || "geral").toLowerCase();
    document.getElementById("holiday-modal-desc").textContent = data.holiday_desc || "Sem descrição disponível.";
