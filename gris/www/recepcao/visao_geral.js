...
frappe.ready(function() {
    ...
    $('.kanban-card').on('click', function(e) {
        ...
        if (status === 'Novo Contato') {
            ...
            openVisitaAgendadaModal(id, responsavel, nome, responsavelAssociado, ramo, visitaData, visitaConfirmada);
...
function confirmarDesistencia() {
    const motivo = $('#cd_motivo').val();
    
    if (!motivo) {
        frappe.msgprint('Por favor, informe o motivo da desistência.');
        return;
    }
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (!r.exc) {
                frappe.show_alert({message: 'Desistência registrada e dados removidos.', indicator: 'green'});
...
function openVisitaAgendadaModal(id, responsavel, nome, responsavelAssociado, ramo, visitaData, visitaConfirmada) {
    currentCardId = id;
    ...
    if (isConfirmed) {
        $('#va_status_visita').text('Confirmada').addClass('text-success').removeClass('text-muted');
...
function registrarRecepcaoRealizada() {
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (!r.exc) {
                frappe.show_alert({message: 'Recepção realizada! Movido para Aguardar Dados.', indicator: 'green'});
                closeModal();
                setTimeout(() => window.location.reload(), 500);
            }
        }
...
function confirmarRegistroCriado() {
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (!r.exc) {
                frappe.show_alert({message: 'Registro confirmado e movido para Acompanhamento', indicator: 'green'});
                closeModal();
...
function openAcompanhamentoModal(id, responsavel, nome, responsavelAssociado, steps) {
    ...
    if (steps && steps.length > 0) {
        steps.forEach(step => {
            ...
            const clickAttr = (!step.completed && fieldName) ? `onclick="toggleStep('${fieldName}', this)" style="cursor: pointer;"` : '';
            ...
            const html = `
                <div class="timeline-item ${completedClass} ${itemExtraClass} ${hoverClass}" ${clickAttr}>
                    ...
                    <div class="timeline-content">
                        <span class="timeline-label">${labelHtml}</span>
                        ${!step.completed ? '<small class="d-block text-muted" style="font-size: 0.75rem;">Clique para marcar como concluído</small>' : ''}
                    </div>
                </div>
            `;
            timelineContainer.append(html);
        ...
        frappe.call({
            ...
            callback: function(r) {
                if (!r.exc) {
                    ...
                    $(element).addClass('completed').removeAttr('onclick').css('cursor', 'default').removeClass('step-clickable');
