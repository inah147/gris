(function () {
	const METHODS = {
		...
		solicitarAlteracoes:
			"gris.gestao_de_projetos.doctype.projeto.projeto.solicitar_alteracoes_projeto",
			...
			.map((stage) => {
				...
				return `
                    <article class="${classes.join(" ")}">
                        ...
                        <p class="approval-flow__meta"><strong>Status:</strong> ${escapeHtml(
							statusLabel
						)}</p>
                        <p class="approval-flow__meta"><strong>Aprovadores elegíveis:</strong> ${escapeHtml(
							approvers || "Não definido"
						)}</p>
                    </article>
                `;
			})
	...
	function updateDecisionUi(approval, status) {
		...
		if (hint) {
			if (status !== "Em aprovacao") {
				...
				hint.textContent = `Você pode decidir a etapa atual: ${
	...
	function fillProjeto(projeto, approval) {
		...
		renderTableRows("view_rows_equipe", projeto.equipe_de_interesse || [], [
			"nome",
			"email",
			"telefone",
			"funcao",
	...
	function openAlteracoesModal() {
		document.getElementById("alteracoesModal")?.showModal();
	}

	function openAprovarEtapaModal() {
		document.getElementById("aprovarEtapaModal")?.showModal();
	}

	function closeAprovarEtapaModal() {
		document.getElementById("aprovarEtapaModal")?.close();
	}

	function closeAlteracoesModal() {
		document.getElementById("alteracoesModal")?.close();
	}
	...
	function bindEvents() {
		...
		if (confirmCancelProjectBtn)
			confirmCancelProjectBtn.addEventListener("click", cancelarProjeto);
